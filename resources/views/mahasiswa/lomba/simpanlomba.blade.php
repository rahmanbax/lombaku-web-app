    ...
    <template id="lomba-card-template">
        <div class="bg-white rounded-lg shadow-md overflow-hidden transform transition-transform duration-300 hover:-translate-y-1">
            ...
            <div class="p-5">
                ...
                <a class="lomba-link-detail mt-5 block w-full bg-blue-600 text-white text-center font-semibold py-2 rounded-lg hover:bg-blue-700">Lihat Detail</a>
            </div>
        </div>
    </template>
            ...
            try {
                ...
                if (response.data.success && response.data.data.length > 0) {
                    response.data.data.forEach(lomba => {
                        ...
                        const detailUrl = `/lomba/${lomba.id_lomba}`;
                        ...
                        card.querySelector('.lomba-penyelenggara').textContent = `Oleh: ${lomba.penyelenggara || 'N/A'}`;
                        card.querySelector('.lomba-tanggal').textContent = 'Batas: ' + new Date(lomba.tanggal_akhir_registrasi).toLocaleDateString('id-ID', {
                            day: 'numeric',
                            month: 'long',
                            year: 'numeric'
                        });

                        // Set link untuk gambar dan tombol
                        card.querySelector('.lomba-img-link').href = detailUrl;
                        card.querySelector('.lomba-link-detail').href = detailUrl;
                ...
                if (error.response && error.response.status === 401) {
                    lombaGrid.innerHTML = `<div class="col-span-full text-center text-gray-500 py-10">
                <p>Anda harus login untuk melihat lomba yang disimpan.</p>
                <a href="/login" class="text-blue-600 hover:underline mt-2 inline-block">Login Sekarang</a>
            </div>`;
                } else {
                    lombaGrid.innerHTML = `<div class="col-span-full text-center text-red-500 py-10">Gagal memuat data. Silakan coba lagi nanti.</div>`;
                }
            }
        });
    </script>
